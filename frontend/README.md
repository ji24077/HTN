...
terminals, using the same `uv run --project backend --python 3.12 --extra demo` prefix. The built
...
Open **http://127.0.0.1:5173**. Vite proxies `/v1`, `/auth`, and `/demo/session` to
...
The demo endpoint remains loopback-only. Public Supabase login uses the separate
public server described in [networking setup](../docs/networking.md).
