...
HIP_SOURCE = r"""#include <cuda_runtime.h>
#include <cstddef>
#include <cstdio>
...
static bool record_status(cudaError_t code, const char* stage) {
    if (code == cudaSuccess) return true;
    std::snprintf(failure_message, sizeof(failure_message), "%s: %s",
                  stage, cudaGetErrorString(code));
    return false;
}
...
extern "C" int portable_transform(const float* x, const float* y, float* output,
    ...
    if (!record_status(cudaMalloc(reinterpret_cast<void**>(&device_storage), 3 * bytes),
    ...
    do {
        ...
        if (!record_status(cudaMemcpyAsync(device_x, x, bytes, cudaMemcpyHostToDevice, 0), "copy x") ||
            !record_status(cudaMemcpyAsync(device_y, y, bytes, cudaMemcpyHostToDevice, 0), "copy y")) break;
        const unsigned int required = static_cast<unsigned int>((n + 127) / 128);
        const unsigned int groups = required < 128 ? required : 128;
        grid_stride_axpy<<<dim3(groups), dim3(128), 0, 0>>>(device_x, device_y, device_output, n, alpha);
        if (!record_status(cudaGetLastError(), "launch AXPY")) break;
        // Blocking D2H copy on the same stream also synchronizes the kernel.
        if (!record_status(cudaMemcpy(output, device_output, bytes, cudaMemcpyDeviceToHost),
                           "copy result")) break;
        status = 0;
    } while (false);
    const cudaError_t release = cudaFree(device_storage);
    if (release != cudaSuccess && status == 0) {
