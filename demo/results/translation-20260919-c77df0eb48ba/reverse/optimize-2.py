"""Pasteable independently authored HIP-to-CUDA translation demo: custom GPU preprocessing + PyTorch training.

No API keys, external datasets, model downloads, or repository imports are needed.
The embedded HIP source must actually be translated before running on NVIDIA.
...
# This independent HIP-first implementation is the AMD-to-NVIDIA input fixture.
# A different PyTorch wheel or compiler does not itself translate this source.
HIP_SOURCE = r"""#include <hip/hip_runtime.h>
#include <cstddef>
#include <cstdio>

// Stateless HIP-first AXPY: one device allocation per call, fully released on
// every exit path (including errors). No device buffers persist between calls.
// The only static object is the host-side diagnostic string mandated by the C ABI.
static char failure_message[512] = "";

static bool record_status(hipError_t code, const char* stage) {
    if (code == hipSuccess) return true;
    std::snprintf(failure_message, sizeof(failure_message), "%s: %s",
                  stage, hipGetErrorString(code));
    return false;
}

__global__ void grid_stride_axpy(const float* input, const float* addend,
                                 float* result, std::size_t length, float scale) {
    const std::size_t first = static_cast<std::size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    const std::size_t stride = static_cast<std::size_t>(gridDim.x) * blockDim.x;
    for (std::size_t index = first; index < length; index += stride) {
        result[index] = scale * input[index] + addend[index];
    }
}

extern "C" const char* portable_last_error() { return failure_message; }

extern "C" int portable_transform(const float* x, const float* y, float* output,
                                  std::size_t n, float alpha) {
    failure_message[0] = '\0';
    if (n == 0) return 0;
    // Guard against overflow of n * 3 * sizeof(float) on 32-bit size_t.
    if (!x || !y || !output ||
        n > (static_cast<std::size_t>(-1) / (3 * sizeof(float))) ||
        n > static_cast<std::size_t>(2147483647)) {
        std::snprintf(failure_message, sizeof(failure_message), "invalid host buffers or length");
        return 1;
    }
    const std::size_t bytes = n * sizeof(float);
    float* device_storage = nullptr;
    if (!record_status(hipMalloc(reinterpret_cast<void**>(&device_storage), 3 * bytes),
                       "allocate AXPY buffers")) {
        return 1; // nothing else owned yet
    }
    float* device_x = device_storage;
    float* device_y = device_storage + n;
    float* device_output = device_storage + 2 * n;
    int status = 1;
    do {
        // Overlap-free staging: enqueue both H2D copies, then the kernel, on the
        // default stream; they execute in issue order so no extra sync is needed.
        if (!record_status(hipMemcpyAsync(device_x, x, bytes, hipMemcpyHostToDevice, 0), "copy x") ||
            !record_status(hipMemcpyAsync(device_y, y, bytes, hipMemcpyHostToDevice, 0), "copy y")) break;
        const unsigned int required = static_cast<unsigned int>((n + 127) / 128);
        const unsigned int groups = required < 128 ? required : 128;
        hipLaunchKernelGGL(grid_stride_axpy, dim3(groups), dim3(128), 0, 0,
                           device_x, device_y, device_output, n, alpha);
        if (!record_status(hipGetLastError(), "launch AXPY")) break;
        // Blocking D2H copy on the same stream also synchronizes the kernel.
        if (!record_status(hipMemcpy(output, device_output, bytes, hipMemcpyDeviceToHost),
                           "copy result")) break;
        status = 0;
    } while (false);
    const hipError_t release = hipFree(device_storage);
    if (release != hipSuccess && status == 0) {
        record_status(release, "release AXPY buffers");
        status = 1;
    }
    return status;
}
...
def build() -> Path:
    """Compile the embedded source; the initial source is HIP, not native CUDA."""
...
def main() -> None:
    ...
    report = {
        ...
        "precision": "float32", "training_config": TRAINING_CONFIG,
        "native_kernel_source_sha256": hashlib.sha256(HIP_SOURCE.encode()).hexdigest(),
