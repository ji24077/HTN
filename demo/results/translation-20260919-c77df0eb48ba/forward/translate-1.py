...
CUDA_SOURCE = r"""#include <hip/hip_runtime.h>
#include <cstddef>
#include <cstdio>
...
static int release_buffers(float* a, float* b, int status) {
    float* allocations[] = {a, b};
    for (float* allocation : allocations) {
        if (!allocation) continue;
        hipError_t result = hipFree(allocation);
        if (result != hipSuccess && status == 0) {
            checked(result, "hipFree");
            status = 1;
        }
    }
    return status;
...
extern "C" int portable_transform(const float* x, const float* y, float* output,
    ...
    // Only two device buffers are needed: x is staged in device_x and the
    // kernel writes the result in place, avoiding one allocation and copy.
    float *device_x = nullptr, *device_y = nullptr;
    const std::size_t bytes = n * sizeof(float);
    if (!checked(hipMalloc(reinterpret_cast<void**>(&device_x), bytes), "hipMalloc x") ||
        !checked(hipMalloc(reinterpret_cast<void**>(&device_y), bytes), "hipMalloc y")) {
        return release_buffers(device_x, device_y, 1);
    }
    if (!checked(hipMemcpy(device_x, x, bytes, hipMemcpyHostToDevice), "hipMemcpy x") ||
        !checked(hipMemcpy(device_y, y, bytes, hipMemcpyHostToDevice), "hipMemcpy y")) {
        return release_buffers(device_x, device_y, 1);
    }
    const unsigned int blocks = static_cast<unsigned int>((n + 255) / 256);
    // In-place axpy: device_x holds x, device_y holds y and receives the result.
    hipLaunchKernelGGL(axpy_kernel, dim3(blocks), dim3(256), 0, 0,
                       device_x, device_y, device_y, n, alpha);
    if (!checked(hipGetLastError(), "axpy launch") ||
        !checked(hipDeviceSynchronize(), "axpy synchronize") ||
        !checked(hipMemcpy(output, device_y, bytes, hipMemcpyDeviceToHost),
                 "hipMemcpy output")) {
        return release_buffers(device_x, device_y, 1);
    }
    return release_buffers(device_x, device_y, 0);
}
"""
