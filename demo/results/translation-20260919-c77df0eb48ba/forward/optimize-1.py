...
extern "C" int portable_transform(const float* x, const float* y, float* output,
    ...
    // Only two device buffers are needed: x is staged in device_output and the
    ...
    axpy_kernel<<<blocks, 256>>>(device_x, device_y, device_y, n, alpha);
    if (!checked(cudaGetLastError(), "axpy launch") ||
        !checked(cudaDeviceSynchronize(), "axpy synchronize") ||
        !checked(cudaMemcpy(output, device_y, bytes, cudaMemcpyDeviceToHost),
                 "cudaMemcpy output")) {
        return release_buffers(device_x, device_y, 1);
    }
