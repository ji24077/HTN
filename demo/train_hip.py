...
// Independently authored HIP-first AXPY with a single device allocation.
...
extern "C" int portable_transform(const float* x, const float* y, float* output,
    ...
    if (!record_status(hipMalloc(reinterpret_cast<void**>(&device_storage), 3 * bytes), "allocate AXPY buffers")) {
        return 1;
    }
    ...
    int status = 1;
    do {
        if (!record_status(hipMemcpy(device_x, x, bytes, hipMemcpyHostToDevice), "copy x") ||
            !record_status(hipMemcpy(device_y, y, bytes, hipMemcpyHostToDevice), "copy y")) break;
        // A capped grid exercises the stride independently of the NVIDIA fixture.
        const unsigned int required = static_cast<unsigned int>((n + 127) / 128);
        const unsigned int groups = required < 64 ? required : 64;
        hipLaunchKernelGGL(grid_stride_axpy, dim3(groups), dim3(128), 0, 0,
                           device_x, device_y, device_output, n, alpha);
        if (!record_status(hipGetLastError(), "launch AXPY") ||
            !record_status(hipDeviceSynchronize(), "finish AXPY") ||
            !record_status(hipMemcpy(output, device_output, bytes, hipMemcpyDeviceToHost), "copy result")) break;
        status = 0;
    } while (false);
    const hipError_t release = hipFree(device_storage);
    // Keep the first failure; cleanup failures still fail an otherwise good call.
