...
extern "C" int portable_transform(const float* x, const float* y, float* output,
    ...
    if (!checked(hipMalloc(reinterpret_cast<void**>(&device_x), bytes), "cudaMalloc x") ||
        !checked(hipMalloc(reinterpret_cast<void**>(&device_y), bytes), "cudaMalloc y") ||
        !checked(hipMalloc(reinterpret_cast<void**>(&device_output), bytes), "cudaMalloc output")) {
        return release_buffers(device_x, device_y, device_output, 1);
    }
    if (!checked(hipMemcpy(device_x, x, bytes, hipMemcpyHostToDevice), "cudaMemcpy x") ||
        !checked(hipMemcpy(device_y, y, bytes, hipMemcpyHostToDevice), "cudaMemcpy y")) {
        return release_buffers(device_x, device_y, device_output, 1);
    }
    ...
    if (!checked(hipGetLastError(), "axpy launch") ||
        ...
        !checked(hipMemcpy(output, device_output, bytes, hipMemcpyDeviceToHost),
                 "cudaMemcpy output")) {
        return release_buffers(device_x, device_y, device_output, 1);
    }
