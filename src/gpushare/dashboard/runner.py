...
class JobManager:
    ...
    def create(
        ...
        with self._lock:
            # A new job stays queued until its worker thread acquires this lock.
            # Counting queued as busy closes the window where two button clicks
            # could otherwise launch GPU work on the same pod concurrently.
            busy = next(
                (j for j in self._jobs.values() if j.status in ("running", "queued")), None
            )
            if busy:
...
def _free_vram_gb(info: dict[str, Any]) -> float:
    """Read free accelerator memory on CUDA or ROCm hosts.

    ``nvidia-smi`` was previously unconditional, so an AMD pod could pass the
    migration preflight and then be rejected by placement before any workload
    ran.  The Torch fallback uses the image's native ROCm environment and emits
    bytes in a deliberately machine-readable form.
    """
    command = (
        ...
        "/opt/venv/bin/python -c \"import torch; print(torch.cuda.mem_get_info()[0])\"; "
        "else python3 -c \"import torch; print(torch.cuda.mem_get_info()[0])\"; fi"
    )
    out = _capture(_ssh_args(info, command))
    values = [int(x) for x in re.findall(r"(?m)^\s*(\d+)\s*$", out)]
    return min(values, default=0) / 1e9
        ...
        return (
            _capture(
                _ssh_args(
                    ...
                    "elif command -v rocm-smi >/dev/null 2>&1; then rocm-smi --showpids; fi",
                )
            )
            .strip()
            .replace("\n", "; ")
...
def _training_agent_candidates(vendor: str) -> list[dict[str, Any]]:
    """Executable training search space for one accelerator backend."""
    candidates = [
        {"method": "lora", "micro_batch": 16, "grad_accum": 1,
         "gradient_checkpointing": False, "lora_rank": 8},
        {"method": "lora", "micro_batch": 8, "grad_accum": 2,
         "gradient_checkpointing": True, "lora_rank": 8},
    ]
    if vendor == "nvidia":
        candidates += [
            {"method": "qlora", "micro_batch": 16, "grad_accum": 1,
             "gradient_checkpointing": False, "lora_rank": 8},
            {"method": "qlora", "micro_batch": 8, "grad_accum": 2,
             "gradient_checkpointing": True, "lora_rank": 8},
        ]
    elif vendor != "amd":
        raise JobError(f"training optimizer has no backend for vendor {vendor}")
    return candidates


def _inference_agent_candidates(vendor: str, *, adapter: bool) -> list[dict[str, Any]]:
    """Executable runtime/precision search space, excluding unsupported paths."""
    candidates = [
        {"name": "eager-b8-bf16", "runtime": "eager", "batch": 8,
         "quantization": "bf16", "kv_cache": True, "fuse_adapter": False},
        {"name": "eager-b16-bf16", "runtime": "eager", "batch": 16,
         "quantization": "bf16", "kv_cache": True, "fuse_adapter": False},
        {"name": "compile-b16-bf16", "runtime": "compile", "batch": 16,
         "quantization": "bf16", "kv_cache": True, "fuse_adapter": False},
    ]
    if vendor == "nvidia":
        candidates += [
            {"name": "eager-b16-int8", "runtime": "eager", "batch": 16,
             "quantization": "int8", "kv_cache": True, "fuse_adapter": False},
            {"name": "eager-b16-nf4", "runtime": "eager", "batch": 16,
             "quantization": "nf4", "kv_cache": True, "fuse_adapter": False},
        ]
    elif vendor != "amd":
        raise JobError(f"inference optimizer has no backend for vendor {vendor}")
    if adapter:
        candidates.append(
            {"name": "eager-b16-fused", "runtime": "eager", "batch": 16,
             "quantization": "bf16", "kv_cache": True, "fuse_adapter": True}
        )
    return candidates
...
def start_training_optimization(*, pod_id: str) -> Job:
    def work(job: Job) -> dict[str, Any]:
        ...
        # Every candidate keeps the same effective batch.  NVIDIA can measure
        # both LoRA and real NF4 QLoRA; bitsandbytes does not support the ROCm
        # path used by these pods, so AMD evaluates LoRA candidates only.
        candidates = _training_agent_candidates(pod["vendor"])
        remote_base = f"{REMOTE_ROOT}/.runs/{job.id}/training-bench"
        measured = []
        for idx, candidate in enumerate(candidates):
            mb, ga = candidate["micro_batch"], candidate["grad_accum"]
            method = candidate["method"]
            JOBS.update(
                job,
                f"measuring {method} batch {mb} x accum {ga}",
                20 + int(idx * 60 / len(candidates)),
            )
            name = f"{method}-mb{mb}-ga{ga}-gc{int(candidate['gradient_checkpointing'])}"
            code = _remote(
                job,
                info,
                [
            ...
            measured.append({"name": name, **candidate, "code": code})
        ...
        for row in measured:
            ...
            if row["code"] == 0 and path.exists():
                ...
                if math.isfinite(meta["final_loss"]):
                    step_cost = meta["t_step_median_s"] * float(pod.get("cost_per_hour") or 0) / 3600
                    valid.append({
                        **row,
                        **meta,
                        "tokens_per_second": meta["tokens_per_step"] / meta["t_step_median_s"],
                        "estimated_cost_per_step_usd": step_cost,
                    })
        if not valid:
        ...
        # Same pod and same token work means fastest is also cheapest per job.
        # Keep the monetary objective explicit so a later multi-provider route
        # can compare candidates without changing the result contract.
        chosen = min(valid, key=lambda x: (x["estimated_cost_per_step_usd"], x["t_step_median_s"]))
        selection = {
        ...
        applied = validation.get("status") == "ok" and quality is not None
        remote_checkpoint = f"{REMOTE_ROOT}/.runs/{job.id}/validate/ckpt" if applied else None
        ...
        return {
            ...
            "validation": validation,
            "applied": applied,
            "outcome": "lower-cost stable training" if applied else "candidate rejected by quality gate",
...
def _validate_selection(
    ...
    code = _remote(
        ...
        [
            ...
            selection["method"],
            "--lora-rank",
            str(selection["lora_rank"]),
            "--micro-batch",
            str(selection["micro_batch"]),
            "--grad-accum",
            str(selection["grad_accum"]),
            "--gradient-checkpointing"
            if selection["gradient_checkpointing"]
            else "--no-gradient-checkpointing",
            "--checkpoint-every",
            str(max(1, steps // 4)),
        ],
...
def start_inference_optimization(*, pod_id: str) -> Job:
    def work(job: Job) -> dict[str, Any]:
        ...
        _setup_pod(job, info, pod["vendor"])
        # Runtime, batching, decode KV caching and quantization are independent
        # levers.  Each subprocess gets a fresh model so a failed compile or
        # quantizer cannot contaminate the following measurement.
        local_checkpoint = Path(checkpoint["local_dir"]) / "ckpt"
        candidates = _inference_agent_candidates(
            pod["vendor"], adapter=(local_checkpoint / "adapter_config.json").is_file()
        )

        remote_model = checkpoint["remote_checkpoint"].replace(f"{REMOTE_ROOT}/", "")
        attempted = []
        for index, candidate in enumerate(candidates):
            JOBS.update(
                job,
                f"measuring {candidate['runtime']} / {candidate['quantization']} / "
                f"batch {candidate['batch']}",
                25 + int(index * 55 / len(candidates)),
            )
            argv = [
                "uv", "run", "python", "scripts/benchmark_inference.py",
                "--model", remote_model,
                "--out", f".runs/{job.id}/{candidate['name']}.json",
                "--n", "64",
                "--batch", str(candidate["batch"]),
                "--runtime", candidate["runtime"],
                "--quantization", candidate["quantization"],
            ]
            if candidate["fuse_adapter"]:
                argv.append("--fuse-adapter")
            code = _remote(job, info, argv, allow_failure=True)
            attempted.append({**candidate, "code": code})

        local = RUN_ROOT / job.id
        _pull(job, info, f"{REMOTE_ROOT}/.runs/{job.id}", local)
        reports = []
        for candidate in attempted:
            path = local / f"{candidate['name']}.json"
            if candidate["code"] == 0 and path.exists():
                reports.append({**candidate, **_json(path)})
        if not reports:
            raise JobError("every inference runtime/quantization candidate failed")

        # The first eager BF16 report supplies one immutable control:
        # sequential, batch=1 and KV cache disabled.
        reference = next(
            (r for r in reports if r["runtime"] == "eager" and r["quantization"] == "bf16"),
            reports[0],
        )["baseline"]
        accepted, rejected = [], []
        hourly = float(pod.get("cost_per_hour") or 0)
        for report in reports:
            optimized = report["optimized"]
            gate = _quality(reference, optimized)
            same_tokens = optimized["generated_tokens"] == reference["generated_tokens"]
            row = {
                **report,
                "quality_gate": gate,
                "same_generated_tokens": same_tokens,
                "cost_per_million_tokens_usd": (
                    hourly / optimized["tokens_per_second"] * 1_000_000 / 3600
                    if hourly and optimized["tokens_per_second"] > 0 else None
                ),
            }
            if gate["status"] == "ok" and same_tokens:
                accepted.append(row)
            else:
                rejected.append(row)
        if not accepted:
            raise JobError("all inference candidates failed the fixed-work quality gate")

        chosen = max(accepted, key=lambda r: r["optimized"]["tokens_per_second"])
        speedup = chosen["optimized"]["tokens_per_second"] / reference["tokens_per_second"]
        applied = speedup > 1.0
        return {
            "pod": pod,
            "provider": pod.get("provider", "runpod"),
            "baseline": reference,
            "optimized": chosen["optimized"],
            "chosen": {k: chosen[k] for k in (
                "name", "runtime", "batch", "quantization", "fuse_adapter",
                "cost_per_million_tokens_usd",
            )},
            "candidates": accepted,
            "rejected": rejected,
            "speedup": speedup,
            "applied": applied,
            "outcome": "lower latency and higher throughput" if applied else "no faster valid candidate",
            "routing": {
                "selected_provider": pod.get("provider", "runpod"),
                "selected_pod_id": pod["id"],
                "reason": "checkpoint-local route with the highest quality-gated throughput",
            },
            "validation": {
                **chosen["quality_gate"],
