...
[`shared/protocol.py`](../src/orchestrator/shared/protocol.py) contains:
...
`uv run orchestrator tools` or `uv run orchestrator --demo workers`.

All `/v1` client endpoints require the admin bearer token (or the local demo
cookie under the loopback/origin restrictions above). `/v1/worker` requires
