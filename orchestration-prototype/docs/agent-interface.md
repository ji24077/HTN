...
Run these from the prototype directory with the demo server and workers running:

```sh
uv run orchestrator tools
uv run orchestrator --demo workers
uv run orchestrator --demo submit examples/tasks.json
uv run orchestrator --demo task demo-frame-0001
uv run orchestrator --demo wait demo-frame-0001 --timeout 120
uv run orchestrator --demo events --after 0
```

To cancel explicitly:

```sh
uv run orchestrator --demo cancel demo-frame-0001
```
...
```sh
uv run orchestrator --demo call get_task <<'JSON'
