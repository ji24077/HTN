...
async function updateBinary(cfg: AgentConfig, opts: { force?: boolean }): Promise<UpdateResult> {
  type Index = {
    manifest: { version: string; sha256: string; bytes: number; createdAt: string; notes: string }
    signature: string; publicKey: string
    binaries: { target: string; os: string; arch: string; file: string; sha256: string }[]
  }
  ...
  if (!verifyRelease({ manifest: index.manifest, signature: index.signature, publicKey: index.publicKey }, pinned)) {
    log.error('update.signature_rejected', { version: index.manifest.version })
    return { status: 'refused', reason: 'the binary list was not signed by the key this computer pinned — refusing to install it' }
  }
