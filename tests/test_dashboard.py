...
def test_optimization_agents_expose_every_supported_lever():
    training = runner._training_agent_candidates("nvidia")
    assert {row["method"] for row in training} == {"lora", "qlora"}
    assert {row["micro_batch"] * row["grad_accum"] for row in training} == {16}
    assert {row["gradient_checkpointing"] for row in training} == {False, True}
    assert {row["method"] for row in runner._training_agent_candidates("amd")} == {"lora"}

    inference = runner._inference_agent_candidates("nvidia", adapter=True)
    assert {row["runtime"] for row in inference} == {"eager", "compile"}
    assert {row["quantization"] for row in inference} == {"bf16", "int8", "nf4"}
    assert all(row["kv_cache"] for row in inference)
    assert any(row["fuse_adapter"] for row in inference)
    assert {row["quantization"] for row in runner._inference_agent_candidates("amd", adapter=False)} == {"bf16"}


def test_gpu_memory_probe_has_cuda_and_rocm_paths(monkeypatch):
    commands = []
    monkeypatch.setattr(runner, "_ssh_args", lambda info, command: commands.append(command) or [command])
    monkeypatch.setattr(runner, "_capture", lambda args, **kwargs: "24000000000\n")

    assert runner._free_vram_gb({}) == 24.0
    assert "nvidia-smi" in commands[0]
    assert "/opt/venv/bin/python" in commands[0]
