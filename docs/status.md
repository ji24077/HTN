...
> **Integration update (2026-09-19):** the inventory and “what to do next”
> sections below are the original experiment log, not the current code state.
> The dashboard, persistent jobs, serving, full checkpoint migration and all
> three optimization actions now exist. The integrated CPU/local suite passes
> 603 tests (hardware-only cases skip), and the main orchestration backend
> passes 207 tests. New QLoRA and INT8/NF4/compile candidates are intentionally
> not marked verified here: the agent measures them on the selected live pod,
> records unsupported candidates as failures, and applies no result unless the
> same held-out parse/exact quality gate passes.

| Agent | Executed search | Apply condition |
|---|---|---|
| Training Optimization | LoRA/QLoRA, batch×accum, checkpointing, GPU placement/cost | fixed token work, finite loss, same held-out quality |
| Inference Optimization | eager/compile, batching, decode KV, BF16/INT8/NF4, checkpoint-local routing | same generated tokens, parse/exact within 2%, measured speedup |
| Chip Migration | NVIDIA↔AMD resume or weights-only NVIDIA generation move | same evaluation identity and inference settings, parse/exact within 2% |
