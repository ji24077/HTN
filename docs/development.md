...
Run all commands from the repository root. Python source, tests, and package
configuration live in `backend/`; the website lives in `frontend/`. Container
and proxy configuration live in `deploy/`.
...
For a public dashboard, use the separate `orchestrator-public` command with
Supabase Auth and `orchestrator-worker-gateway` behind Tailscale. See
[networking setup](networking.md). Public login is separate from demo cookies.
...
`backend/uv.lock`. The local Python demo has been exercised; the container path below
...
```sh
docker compose --env-file .env.local -f deploy/compose.yaml up --build -d
```

The Compose command reads `.env.local` and enrolls `worker-1`. It exposes the API on loopback port
...
In another terminal in this directory, start a worker using the same `.env.local`:

```sh
uv run --project backend --frozen --env-file .env.local orchestrator-worker
```

To run the server directly instead of in a container, first start only its
dependencies, then use the local URLs in `.env.local`:

```sh
docker compose --env-file .env.local -f deploy/compose.yaml up -d postgres redis
uv run --project backend --frozen --env-file .env.local orchestrator-server
```

The Python modules read environment variables. `uv --env-file` and Compose load
`.env.local`; importing the package opens no database or network connections. The
...
npm --prefix frontend run build
uv sync --project backend --python 3.12 --extra demo
...
uvx ruff format --config backend/pyproject.toml --check backend/src backend/tests examples/agent_task.py
uv build --project backend
```

`backend/tests/unit/` contains isolated client and asset-routing tests. The detailed
...
`backend/src/orchestrator/server/web/`. Build it before `uv build --project backend` or backend asset tests.
...
Python paths below are relative to `backend/src/orchestrator/`.
...
`backend/tests/unit/`; add service-backed suites separately when those scenarios are
