...
def load(model_ref: str, dtype: str):
    ...
    tok = AutoTokenizer.from_pretrained(
        model_ref if (p / "tokenizer_config.json").exists() else MODEL_ID if (ours or adapter) else model_ref,
        padding_side="left",
        local_files_only=p.exists(),
    )
    ...
    if adapter:
        ...
        cfg = PeftConfig.from_pretrained(model_ref, local_files_only=True)
        base = AutoModelForCausalLM.from_pretrained(cfg.base_model_name_or_path, dtype=td)
