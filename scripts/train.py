...
def validate_init_adapter(path: Path | None, *, method: str) -> dict | None:
    ...
    if method not in {"lora", "qlora"}:
        raise ValueError("--init-adapter requires --method lora or qlora")
...
def main() -> None:
    ...
    a.lr = a.lr if a.lr is not None else (2e-4 if a.method in {"lora", "qlora"} else 1e-5)
    ...
    model_kwargs = {}
    if resume_manifest and a.method in {"lora", "qlora"} and resume_manifest.get("base_model_revision"):
        model_kwargs["revision"] = resume_manifest["base_model_revision"]
    if a.method == "qlora":
        if torch.version.hip is not None:
            ap.error("QLoRA currently requires an NVIDIA CUDA pod; use LoRA on AMD")
        try:
            from transformers import BitsAndBytesConfig
        except ImportError as exc:
            ap.error(f"QLoRA requires bitsandbytes: {exc}")
        model_kwargs["quantization_config"] = BitsAndBytesConfig(
            load_in_4bit=True,
            bnb_4bit_quant_type="nf4",
            bnb_4bit_use_double_quant=True,
            bnb_4bit_compute_dtype=DTYPES[a.dtype],
        )
    ...
    meta = {
        ...
        **token_work,
        ...
        "lr": a.lr,
        "lora_rank": model.peft_config["default"].r if a.method in {"lora", "qlora"} else None,
        "base_weight_quantization": "nf4" if a.method == "qlora" else None,
