...
from .db.store import EnrollmentLimit
...
async def enroll(request: Request):
    ...
    if issuer is None:
        raise HTTPException(status_code=503, detail="Automatic worker enrollment is not configured")
    body = bytearray()
    try:
    ...
    key_id = None
    try:
        key_id, auth_key = await issuer.create()
        await store.finish_enrollment(worker_id, key_id)
    except BaseException:
        if key_id:
            await issuer.revoke(key_id)
        await store.finish_enrollment(worker_id, None)
